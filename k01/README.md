...
## ソースコードの説明
L68:int型でposを定義
L69:int型でresultを定義、-1を代入
L71:for文でIDが一致するデータを１つずつ照合していく
L74:一致したとき、resultにその時のposを代入

L85:int型でposを定義
L86:int型でresultを定義、-1を代入
L91:while文でデータを左右に分けて考え、左が右より小さいか等しくなるまで間繰り返す
L94:if文で場合分け
L98:keyが大きい時、pos+1をleftに代入
L100:keyが小さい時、pos-1をrightに代入
